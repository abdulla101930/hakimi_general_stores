...
export const buildCheckoutWhatsAppText = (
  ...
  msg += `💳 *Payment Method:* ${order.paymentMethod === 'ONLINE' ? '🟢 ONLINE PAYMENT (UPI)' : '💵 CASH ON DELIVERY (COD)'}\n`;
  msg += `📊 *Payment Status:* ${order.paymentStatus || 'Pending'} ${paymentDetails ? `(${paymentDetails})` : ''}\n\n`;
  msg += `📦 *ORDER ITEMS:*\n`;
